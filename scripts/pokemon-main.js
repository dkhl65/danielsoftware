$(document).ready(() => {
  ...
  for (let i = 0; i < TYPES.length; i++) {
    $(".type-select, .attack-select").append(
      `<option value="${i}">${
        TYPES[i][0].toUpperCase() + TYPES[i].substring(1)
      }</option>`
    );
    if (i > 0 && i % 6 === 0) {
    ...
    $(".type-chart").append(
      `<span class="matchup-span"><img class="matchup-img" src="images/icons/${TYPES[i]}.png"></img><label class="icon-label"></label></span>`
    );
  }
  ...
  for (let i = 0; i < ABILITIES.length; i++) {
    $(".ability-select").append(
      `<option value="${i}">${ABILITIES[i]}</option>`
    );
  }
  ...
  $(".type-column").change(() => {
    ...
    $(".pokemon-row").each((a, row) => {
      ...
      for (let i = 0; i < TYPES.length; i++) {
        ...
        // ability immunities
        if (
          (ability >= 0 && ability <= 2 && i === TYPES.indexOf("electric")) ||
          (ability >= 3 && ability <= 5 && i === TYPES.indexOf("water")) ||
          (ability >= 7 && ability <= 8 && i === TYPES.indexOf("fire")) ||
          (ability >= 12 && ability <= 13 && i === TYPES.indexOf("ground")) ||
          (ability === ABILITIES.indexOf("Sap Sipper") &&
            i === TYPES.indexOf("grass"))
        ) {
          $(row)
            .find(".resistances-column")
            .find(".matchup-img")
            .eq(i)
            .css("opacity", "1");
          $(row)
        ...
        // ability resistances
        if (
          ability === ABILITIES.indexOf("Dry Skin") &&
          i === TYPES.indexOf("fire")
        ) {
          matchup *= 1.25;
        }
        if (
          ability === ABILITIES.indexOf("Fluffy") &&
          i === TYPES.indexOf("fire")
        ) {
          matchup *= 2;
        }
        ...
        if (
          matchup < 1 ||
          (matchup <= 1 && ability === ABILITIES.indexOf("Wonder Guard"))
        ) {
          $(row)
            .find(".resistances-column")
            .find(".matchup-img")
      ...
      for (let i = 0; i < weaknesses.length; i++) {
        ...
        if (immunities[i] > 0) {
          ...
          $("#total-row")
            ...
            .find(".icon-label")
            .eq(i)
            .html(`${resistances[i]}+${immunities[i]}`);
      ...
      for (let i = 0; i < ineffective.length; i++) {
        ...
        if (types.length > 1) {
          $(row)
            ...
            .append(
              `<div class="coverage-div"><span class="coverage-span"><img class="left-coverage-img" src="images/icons/${
                TYPES[types[0]]
              }.png"></span><span class="coverage-span"><img class="right-coverage-img" src="images/icons/${
                TYPES[types[1]]
              }.png"></span></div> `
            );
        } else {
          $(row)
            .find(".coverage-column")
            .append(
              `<img class="coverage-img" src="images/icons/${
                TYPES[types[0]]
              }.png"> `
            );
        }
